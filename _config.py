...
# 0c. 第五輪：地域擇時（VTI vs 國際），基準是標的池自己的市值組合
#   結構優勢：基準 VT 由標的池本身組成，策略與基準共用絕大部分變異，
#   ΔSharpe 的雜訊 SD 從 13 檔設定的 0.255 降到約 0.06~0.08。
#   動能是固定規則、不需訓練，因此沒有 fold；全樣本皆為樣本外。窗口沿用 MOM_WINDOWS，本輪不調。
R5_DATA_FILE = 'geo_timing.csv'
R5_US = 'VTI'
R5_INTL_PRIMARY = 'VEU'      # 主軌：2007-03 上市，樣本長且涵蓋 GFC 尾段
R5_INTL_CONTROL = 'VXUS'     # 對照軌：2011-01 上市，含小型股，覆蓋較完整
R5_BENCHMARK = 'VT'
R5_TICKERS = ['VTI', 'VEU', 'VXUS', 'VT', 'BIL']
R5_START_DATE = '2006-01-01'
R5_LOOKBACK_DAYS = 252       # 與 PIT 相同的入池門檻
R5_CRISIS_YEARS = [2008, 2011, 2015, 2018, 2020, 2022]
R5_CEILING_VEU = 0.74        # 完美預知全押相對 VT 的 ΔSharpe（開跑前量定，僅作材料性尺規）
R5_CEILING_VXUS = 0.78
R5_MIN_DETECTABLE_FRACTION = 0.20   # 低於天花板此比例即為檢定力不足，判為「無法回答」而非「無效果」
...
# 5e. 第五輪（地域擇時）的 pre-registration。由 _10_geo_timing.py 原文印出並機械化判定。
PRE_REGISTRATION_GEO = """本輪目的：檢驗動能規則在「美股 vs 國際股」這一條軸上**有沒有擇時能力**。
不是找最佳窗口、不是找最佳權重、也不是證明動能能贏大盤。

### 為什麼換這個設定
基準 VT 由標的池（VTI + 國際）本身組成，策略與基準共用絕大部分變異，
因此 ΔSharpe 被量得準得多：擲銅板全押的 ΔSharpe 雜訊 SD 約 0.06（VEU 軌）/ 0.08（VXUS 軌），
而 13 檔設定是 0.255。同樣月數下可偵測的效果小了 3~4 倍。

### 必須先講的陷阱（本輪的核心設計理由）
**在任何可取得的樣本上美股都大勝國際股**（VTI 0.72 vs VEU 0.33 vs VT 0.53，2008-07 起；
VTI 0.91 vs VXUS 0.50 vs VT 0.74，2012-02 起）。動能會絕大多數時間持有 VTI，
因而會贏過 VT——但那是一個剛好付錢的靜態美股傾斜，不是擇時能力。
可偵測門檻 +0.16 與「永遠持有 VTI」的 +0.17 幾乎相同，這說明「贏過 VT」這個門檻
光靠固定傾斜就能跨過。**因此 VT 不是本輪的判準，只作附帶報告。**

### 設計（開跑前寫定，看到結果後不得更動）
標的池：{R5_US, 國際} 兩檔。主軌國際 = VEU，對照軌 = VXUS（雙軌，§3）。
入池門檻：R5_LOOKBACK_DAYS 個交易日歷史。動能分數沿用 MOM_WINDOWS，**本輪不調任何窗口**。
持股規則：**全押動能分數較高的那一檔**（top 1 of 2）。理由是效果量最大、檢定力最高；
  部分傾斜只是把主動部位等比例縮小，資訊比率不變，故不另外測（測第二種即是搜尋）。
無訓練、無 fold：動能是固定規則，全樣本皆為樣本外。
會計沿用既有口徑（月底換股、shift(1)、現金月計 0%、淨 COST_BPS、Sharpe 扣 RF_TICKER）。

### 核心分解：把「平均傾斜」和「擇時」分開
令 a_t ∈ {0,1} 為策略當月的美股權重，w̄ = a_t 的樣本平均。
  擇時報酬 r_timing,t = (a_t − w̄) × (R_US,t − R_intl,t)
主動權重 (a_t − w̄) 的平均恰為 0，**平均傾斜的貢獻被完全移除，剩下的就是純擇時**。
w̄ 取自樣本內是刻意的：它讓對照更難被贏過，方向保守。

### 判準
A（擇時能力，本輪的核心）：r_timing 的均值 > 0 且 t > 2
   → 成立：「動能在地域軸上有擇時能力」；不成立：「無證據」。
B（組合層，對曝險配對的靜態傾斜）：動能與「固定權重 w̄」組合的 ΔSharpe，
   其配對 bootstrap 95% 信賴區間不含 0。
C（對 VT）：一併報告，但**明確不是通過條件**——理由見上面的陷阱說明。
D（方向命中率）：策略每月是否選到次月報酬較高者，對 50% 做二項檢定。
E（分期一致性）：前半/後半、壓力年/非壓力年各報一次。
   任一種切法出現**符號反轉**即依 §3 淘汰該效果，不論全期 t 值多好看。
F（雙軌）：VEU 主軌與 VXUS 對照軌方向必須一致；不一致即淘汰。
G（檢定力誠實條款）：把效果以「佔完美預知天花板的比例」表示
   （天花板 R5_CEILING_*，開跑前已量定）。若觀察到的效果低於
   R5_MIN_DETECTABLE_FRACTION × 天花板，**判為「檢定力不足、無法回答」，而不是「無效果」**。

### 已知的資訊洩漏（誠實標記）
開跑前已量定並已知：各標的與 VT 的 Sharpe、兩軌的完美預知天花板（+0.74 / +0.78）、
最差預知地板、擲銅板全押的 ΔSharpe 雜訊 SD。**尚未計算任何動能在本標的池上的結果。**
「美股在兩個窗口都大勝」這件事我已知，這正是判準以 A/B 為核心、把 C 降為附帶報告的原因。

### 禁止
調整 MOM_WINDOWS 或任何動能參數；改變持股規則（全押 vs 部分傾斜）；
增減標的；看到結果後更換基準或主軌；只報對自己有利的那一軌；事後放寬門檻；
把 G 的「無法回答」改寫成「無效果」或反之。
"""
